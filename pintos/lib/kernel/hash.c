...
/* 해시 테이블 H를 파괴한다.
   ...
   중 어떤 함수라도 해시 테이블 H를 수정하는 행위는 정의되지 않은 동작을
   유발한다.
*/
...
#define MIN_ELEMS_PER_BUCKET \
    1 /* 버킷당 요소 수가 1보다 작으면: 버킷 수를 줄인다. */
#define BEST_ELEMS_PER_BUCKET 2 /* 이상적인 버킷당 요소 수. */
#define MAX_ELEMS_PER_BUCKET \
    4 /* 버킷당 요소 수가 4보다 크면: 버킷 수를 늘린다. */
